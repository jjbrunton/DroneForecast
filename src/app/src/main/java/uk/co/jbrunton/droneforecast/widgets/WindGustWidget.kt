 ...
 */
class WindGustWidget(private val forecastStream: Observable<ForecastResponse>, private val settingsService: SettingsService, private val context: Context) : WeatherWidget {
    ...
    private fun renderWidgetView(forecast: ForecastItemResponse): View {
        val windToConverter = SI.METERS_PER_SECOND.getConverterTo(settingsService.getWindUnit())
        val windGustUnit = windToConverter.convert(Measure.valueOf(forecast.windGust, SI.METRES_PER_SECOND).doubleValue(SI.METRES_PER_SECOND))

        this.dataTextView.text = "%.1f".format(windGustUnit) + settingsService.getWindUnit().toString().toUpperCase()
        return this.view
    }
}